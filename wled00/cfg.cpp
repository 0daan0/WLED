  ...
  */
  ...
  if (!ins.isNull()) {
    ...
    for (unsigned i = 0; i < WLED_MAX_BUSSES; i++) {
      ...
      busConfigs.emplace_back(dataType, defPin, start, count, DEFAULT_LED_COLOR_ORDER, defaultReversed, 0, RGBW_MODE_MANUAL_ONLY, 0, LED_MILLIAMPS_DEFAULT, ABL_MILLIAMPS_DEFAULT, 0); // driver=0 (RMT default)
      doInitBusses = true;  // finalization done in beginStrip()
    }
  }
